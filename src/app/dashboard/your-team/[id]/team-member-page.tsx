...
export default function TeamMemberPage({ id, initialTab = "info" }: Props) {
  ...
  const { employee: member, loading: memberLoading } = useGetEmployeeById(
    id,
    reloadKey,
  );
  ...
  useEffect(() => {
    ...
    const profileImageUrl =
      localMember?.meta?.design?.images?.profile?.url || "";
    ...
    setBranchSchedules(
      buildBranchSchedules(
        ...
        normalizeWorkSchedule(
          Array.isArray(localMember?.work_schedule)
            ? localMember!.work_schedule
            : [],
        ),
      ),
    );
  ...
  const handleToggleService = async (service: Service) => {
    ...
    try {
      const res = await fetch(
        `/api/employee/${localMember.id}/service/${service.id}`,
        {
  ...
  const linkedBranchIds = new Set(
    localMember?.branches?.map(b => b.id.toString()) ?? [],
  );
  ...
  const handleToggleBranch = async (branch: Branch) => {
    ...
    try {
      const res = await fetch(
        `/api/employee/branch/${localMember.id}/branch/${branch.id}`,
        {
          method: isLinked ? "DELETE" : "POST",
        },
      );
  ...
  const memberName = localMember
    ? `${localMember.name || ""} ${localMember.surname || ""}`.trim()
    : "";
  const memberRole = localMember
    ? localMember.role || localMember.permission || "Profissional"
    : "";
        ...
        <div className="mx-auto w-full max-w-4xl p-6 lg:p-8 space-y-6 pb-12">
          ...
          <Tabs defaultValue={initialTab} className="space-y-0">
            <TabsList className="w-full justify-start rounded-xl border border-border bg-muted/30 p-1 h-auto gap-1">
              <TabsTrigger
                value="info"
                className="flex-1 sm:flex-none rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm px-4 py-2 text-sm font-medium transition-all"
              >
                ...
                {isLoading || companyLoading ? (
                  ...
                  <div className="text-center py-12 text-muted-foreground">
                    ...
                    <p className="text-xs mt-1">
                      Crie serviços na seção de serviços antes de vinculá-los.
                    </p>
                  </div>
                ) : (
                  <>
