...
export const GET = auth(async function GET(req, ctx) {
  try {
    ...
    console.log("✅ Sucesso com GET (employee work_range):", responseData);
    return NextResponse.json(responseData, { status: 200 });
  } catch (error) {
    console.error("❌ Erro ao buscar employee work_range:", error);
...
export const PUT = auth(async function PUT(req, ctx) {
  try {
    ...
    console.log(
      "📦 Body recebido (PUT employee work_range):",
    ...
    console.log("✅ Sucesso com PUT (employee work_range):", responseData);
    return NextResponse.json(responseData, { status: 200 });
  } catch (error) {
    console.error("❌ Erro ao atualizar employee work_range:", error);
    return NextResponse.json(
      {
        message: "Erro interno do servidor",
        error: process.env.NODE_ENV === "development" ? error : undefined,
...
export const DELETE = auth(async function DELETE(req, ctx) {
  try {
    ...
    console.log("✅ Sucesso com DELETE (employee work_range):", responseData);
    return NextResponse.json(responseData, { status: 200 });
  } catch (error) {
    console.error("❌ Erro ao deletar employee work_range:", error);
    return NextResponse.json(
      {
        message: "Erro interno do servidor",
        error: process.env.NODE_ENV === "development" ? error : undefined,
