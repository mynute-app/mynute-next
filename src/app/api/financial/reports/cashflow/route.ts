...
export const GET = auth(async function GET(req) {
  try {
    ...
    const result = await fetchFromBackend(req, "/financial/reports/cash-flow", authData.token!, {
      method: "GET",
      queryParams: getQueryParams(req),
    });
    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    return handleApiError(error, "Erro ao buscar relatório de fluxo de caixa.");
  }
});
