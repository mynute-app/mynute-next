...
export const PATCH = auth(async function PATCH(req, ctx) {
  try {
    ...
    const result = await fetchFromBackend(req, `/financial/transactions/${id}/pay`, authData.token!, {
      method: "POST",
      body,
    });
    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    return handleApiError(error, "Erro ao registrar pagamento da transação.");
  }
});
