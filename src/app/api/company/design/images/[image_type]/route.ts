...
export const DELETE = auth(async function DELETE(req, ctx) {
  try {
    console.log("🗑️ Iniciando remoção de imagem da empresa...");
    ...
    const { image_type } = (await ctx.params) as {
      image_type: string;
    };
    ...
    const backendUrl = `${process.env.BACKEND_URL}/company/${authData.companyId}/design/images/${image_type}`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { message: `Erro ao remover a imagem ${image_type} da empresa` },
        { status: response.status }
      );
    }

    console.log(`✅ Imagem ${image_type} da empresa removida com sucesso`);
    return NextResponse.json(
      { message: `Imagem ${image_type} da empresa removida com sucesso` },
      { status: 200 }
    );
  } catch (error) {
    console.error("❌ Erro ao processar remoção da imagem da empresa:", error);
    return NextResponse.json(
      { message: "Erro interno ao remover a imagem da empresa" },
      { status: 500 }
    );
  }
});
